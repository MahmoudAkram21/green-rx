...
class MedicineSuggestionController {
    ...
    async createSuggestion(req, res, next) {
        try {
            const { tradeName, activeSubstance, concentration, dosageForm, manufacturer, reason } = req.body;
            ...
            const suggestion = await prisma_1.prisma.medicineSuggestion.create({
                data: {
                    doctorId: doctor.id,
                    tradeName,
                    activeSubstance,
                    ...
                    manufacturer,
                    reason,
                    status: client_1.SuggestionStatus.Pending
                },
                include: {
                    doctor: {
                        select: {
    ...
    async getSuggestions(req, res, next) {
        try {
            ...
            const suggestions = await prisma_1.prisma.medicineSuggestion.findMany({
                where,
                include: {
                    doctor: {
                        select: {
    ...
    async getSuggestionById(req, res, next) {
        try {
            const { id } = req.params;
            const suggestion = await prisma_1.prisma.medicineSuggestion.findUnique({
                where: { id: Number(id) },
                include: {
                    doctor: {
                        select: {
    ...
    async reviewSuggestion(req, res, next) {
        try {
            const { id } = req.params;
            ...
            const suggestion = await prisma_1.prisma.medicineSuggestion.update({
                where: { id: Number(id) },
                data: {
                    status: status,
                    reviewedBy: userId,
    ...
    async deleteSuggestion(req, res, next) {
        try {
            ...
            if (userRole === 'Doctor') {
                ...
                if (suggestion.status !== client_1.SuggestionStatus.Pending) {
                    res.status(403).json({ error: 'You can only delete pending suggestions' });
                    return;
                }
            }
            await prisma_1.prisma.medicineSuggestion.delete({
                where: { id: Number(id) }
            });
...
exports.default = new MedicineSuggestionController();
//# sourceMappingURL=medicineSuggestion.controller.js.map