...
const paths = {};
const p = (name) => ({ name, in: 'path', required: true, schema: { type: 'integer' } });
const s = (pathKey, method, tag, summary, secure = true, params = []) => {
...
const options = {
    ...
    apis: [path_1.default.join(__dirname, '../routes/*.ts'), path_1.default.join(__dirname, '../controllers/*.ts')]
};
