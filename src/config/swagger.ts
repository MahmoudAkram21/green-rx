...
s('/doctors/{doctorId}/patients/warnings', 'get', [DOCTOR_TAGS.MY_PATIENTS, DOCTOR_PATIENTS_SECTION, DOCTOR_TAGS.DRUG_SAFETY], 'Get all warnings for patients linked to this doctor. Returns { warnings: [ { patientId, name, email, type, severity, message, ... } ] } — one entry per warning with patient info on each.', true, [p('doctorId')]);
